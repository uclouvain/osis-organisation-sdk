...
with osis_organisation_sdk.ApiClient(configuration) as api_client:
    ...
    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.get_entity(uuid)
        pprint(api_response)
    except osis_organisation_sdk.ApiException as e:
    ...
    # and optional values
    try:
        api_response = api_instance.get_entity(uuid, accept_language=accept_language, x_user_first_name=x_user_first_name, x_user_last_name=x_user_last_name, x_user_email=x_user_email, x_user_global_id=x_user_global_id)
        pprint(api_response)
    except osis_organisation_sdk.ApiException as e:
        print("Exception when calling EntitesApi->get_entity: %s\n" % e)
```
...
> PaginatedAddresses get_entity_addresses(organisation_code, uuid)
...
[**PaginatedAddresses**](PaginatedAddresses.md)
